...
import { GymEvent } from '../models/gymEvent.js'
...
router.get('/all', async (_, res) => {
  try {
    res.status(200).json({ results: await GymEvent.find() })
  } catch (error) {
...
router.patch('/update', async (req, res) => {
  try {
    let currentEvent = await GymEvent.findOne(req.query).exec()
    ...
    if (getUserFromToken(req)?._id === currentEvent.user || checkPermissions(req, "Staff")) {
      let event = await GymEvent.findOneAndUpdate(req.query, req.body, { new: true })
...
router.delete('/delete', async (req, res) => {
  try {
    let currentEvent = await GymEvent.findOne(req.query).exec()
    if (getUserFromToken(req)?._id === currentEvent.user || checkPermissions(req, "Staff")) {
      let event = await GymEvent.findOneAndDelete(req.query)
